...
fun FavoriteScreen(
    ...
    favoriteViewModel.favoriteAnime.collectAsState(initial = Resource.Loading()).value.let { state ->
        when (state) {
            ...
            is Resource.Success -> {
                state.data?.let { FavoriteContent(listAnime = it, navigateToDetail = navigateToDetail) }
            }

            is Resource.Error -> {
                ErrorItem(state.message.toString())
            }
        }
    }
}
...
fun FavoriteContent(
    ...
    if (listAnime.isNotEmpty()) {
        GridAnimeList(listAnime = listAnime, navigateToDetail = navigateToDetail)
    } else {
        ErrorItem("There is no data")
    }
}